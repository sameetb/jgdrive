    ...
    public void exec(final Driver driver, final List<String> opts) throws IOException, IllegalStateException
    {
        ...
        if(!opts.contains("local-only"))
        {
            ...
            output = Stream.concat(
                    Stream.concat(remCh.getModifiedDirs(), remCh.getModifiedFiles()).map(f -> "modified: " + ri.getLocalPath(f.getId()).orElse(Paths.get(f.getTitle()))),
                    Stream.concat(remCh.getDeletedDirs(), remCh.getDeletedFiles())
                                    .map(f -> ri.getLocalPath(f.getId()))
                                    .filter(op -> op.isPresent()).map(op -> "deleted: " + op.get()))
                    .map(str -> "remote " + str);
